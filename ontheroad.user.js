...
// @category       Layer
// @version        0.0.1.20160423.001
// @namespace      https://github.com/Hurqalia/ontheroad
// @updateURL      https://github.com/Hurqalia/ontheroad/raw/master/ontheroad.user.js
// @downloadURL    https://github.com/Hurqalia/ontheroad/raw/master/ontheroad.user.js
// @description    [ontheroad-2016-04-23-000001] OnTheRoad
...
function wrapper(plugin_info) {
	...
	plugin_info.dateTimeVersion = '20160423.001';
	...
	'      </div>' +
	'    </div>' +
	'    <div id="infosBoxBottomBar" style="margin:5px; text-align:right;">'+
	'      <button class="otr-infos-button" onclick="window.plugin.ontheroad.hideRouteInfo();">Close</button>' +
	'    </div>' +
	'    </div>';
	...
	window.plugin.ontheroad.addArrivalMark = function () {
		...
		marker.on('click', function(obj) {
			window.plugin.ontheroad.showRouteInfo(obj);
		}, this);
		return marker;
	};

	window.plugin.ontheroad.showRouteInfo = function(obj) {
		ruid = obj.target.options.title;
		$('.otr-infos-content-details').empty();
		$('.otr-infos-content-details').append(window.plugin.ontheroad.routes_infos[ruid].summary);
		$('.otr-infos-content-details').append(window.plugin.ontheroad.routes_infos[ruid].details);
		if (window.plugin.ontheroad.isSmart) {
	...
	window.plugin.ontheroad.portalLatLng = function(pid) {
		return {
			lat: parseFloat(window.plugin.bookmarks.bkmrksObj.portals.idOthers.bkmrk[pid].latlng.split(',')[0]),
			lng: parseFloat(window.plugin.bookmarks.bkmrksObj.portals.idOthers.bkmrk[pid].latlng.split(',')[1]),
		};
	};
	...
	window.plugin.ontheroad.onDialog = function() {
		...
		if ($('.ui-dialog-ontheroad').is(':visible')) {
			return false;
		}
	...
	window.plugin.ontheroad.initCss = function() {
		$('head').append('<style>#ontheroadInfosBox.mobile{ position:absolute !important; width: 100% !important; height: 100% !important; top: 0 !important; left: 0 !important; margin: 0 !important; padding: 0 !important;border: 0 !important; background: transparent !important; overflow:auto !important;} .otr-infos-content-details.mobile { color:black; overflow-y: scroll; height:100%; } .otr-infos-content-details.mobile > div { margin-left:0px!important; } </style>');
 	...
 	window.plugin.ontheroad.initDatas = function() {
		...
		$.each(stored_datas, function(i, datas) {
			setTimeout(function() {
				window.plugin.ontheroad.config_request.label = datas.label;
				window.plugin.ontheroad.config_request.line  = datas.line;
				window.plugin.ontheroad.config_request.uid   = i;
				window.plugin.ontheroad.route_request = datas.request;
	...
	window.plugin.ontheroad.initMobile = function() {
		...
		$('#ontheroadInfosBox').css("display", "none").addClass("mobile");
		$('.otr-infos-content-details').addClass("mobile");
